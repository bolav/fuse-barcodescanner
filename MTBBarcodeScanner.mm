...
- (AVCaptureSession *)newSessionWithCaptureDevice:(AVCaptureDevice *)captureDevice {
    ...
    self.stillImageOutput.outputSettings = [NSDictionary dictionaryWithObjects:@[AVVideoCodecJPEG]forKeys:@[AVVideoCodecKey]];
    ...
    if ([self isCapturingStillImage]) {
        if (captureBlock) {
            NSError *error = [NSError errorWithDomain:kErrorDomain
                                             ...
                                             userInfo:[NSDictionary dictionaryWithObjects:@[@"Still image capture is already in progress. Check with isCapturingStillImage"]forKeys:@[NSLocalizedDescriptionKey]]];
    ...
    if (stillConnection == nil) {
        if (captureBlock) {
            NSError *error = [NSError errorWithDomain:kErrorDomain
                                             ...
                                             userInfo:[NSDictionary dictionaryWithObjects:@[@"AVCaptureConnection is closed"]forKeys:@[NSLocalizedDescriptionKey]]];
            captureBlock(nil, error);
        }
        return;
    }
